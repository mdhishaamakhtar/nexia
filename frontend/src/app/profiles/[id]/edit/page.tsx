...
export default function EditProfilePage() {
    ...
    const fetchProfile = async () => {
        try {
            ...
            setFavoriteMemory(profile.favorite_memory || "");

            setTags(profile.tags?.map((t: any) => t.tag) || []);
            setPoliticalViews(profile.political_views?.map((v: any) => v.view) || []);
            setFoodRestrictions(profile.food_restrictions?.map((r: any) => r.restriction) || []);
            setMovieGenres(profile.movie_genres?.map((g: any) => g.genre) || []);
            setBookGenres(profile.book_genres?.map((g: any) => g.genre) || []);
            setHangoutPlaces(profile.hangout_places?.map((p: any) => p.place) || []);
            setQuotes(profile.quotes?.map((q: any) => q.quote) || []);
            setTopSongs(profile.top_songs || []);

            if (profile.associated_song) {
                setAssociatedSongName(profile.associated_song.name || "");
                setAssociatedSongArtist(profile.associated_song.artist || "");
            }
        } catch (error) {
            ...
            <ProtectedRoute>
                <div className="min-h-screen">
                    ...
                    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        <div className="shimmer-bg h-96 rounded-2xl animate-[shimmer_2s_linear_infinite]" />
                    </div>
                </div>
            </ProtectedRoute>
        );
        ...
        <ProtectedRoute>
            <div className="min-h-screen">
                ...
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <h1 className="text-4xl font-bold mb-8">Update memories</h1>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        {/* Submit */}
                        <Button type="submit" isLoading={isSaving} variant="primary" className="w-full text-lg py-4">
                            Update memories
                        </Button>
                    </form>
                </div>
